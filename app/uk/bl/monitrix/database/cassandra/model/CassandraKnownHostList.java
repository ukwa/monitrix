...
public class CassandraKnownHostList implements KnownHostList {
	...
	// CassandraDB query operator for selecting documents where the field value equals any value in a list
	private static final String MONGO_QUERY_ALL = "$all";
	
	// CassandraDB query operator for selecting documents where the field value is greater or equal to a specified value
	...
	public long countSuccessful() {
		DBObject query = new BasicDBObject(CassandraProperties.FIELD_KNOWN_HOSTS_SUCCESSFULLY_FETCHED_URLS, new BasicDBObject("$exists", true));
		return collection.count(query);
	}
	
	@Override
	public long getMaxFetchDuration() {
		DBCursor cursor = collection.find().sort(new BasicDBObject(CassandraProperties.FIELD_KNOWN_HOSTS_AVG_FETCH_DURATION, -1)).limit(1);
		if (cursor.hasNext()) {
			CassandraKnownHost h = new CassandraKnownHost(cursor.next());
	...
	public boolean isKnown(String hostname) {
		...
		DBObject dbo = collection.findOne(new BasicDBObject(CassandraProperties.FIELD_KNOWN_HOSTS_HOSTNAME, hostname));
		if (dbo == null)
			return false;

		CassandraKnownHost wrapped = new CassandraKnownHost(dbo);
